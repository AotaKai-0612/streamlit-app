...
def analyze_comment(comment_text):
    ...
    for attempt in range(max_retries):
        try:
            ...
            # レート制限エラー(429)の場合は待機してリトライ
            if "429" in str(e):
                time.sleep(2 ** (attempt + 1)) # 2, 4, 8秒待機
                continue
            return {"error": str(e)}
    
    return {"error": "Rate limit exceeded (API制限により失敗)"}
            ...
            for j in range(N_COLS):
                if i + j < len(videos):
                    ...
                    with cols[j]:
                        ...
                        # タイトルが長すぎる場合の省略表示
                        title_disp = v["title"]
                        if len(title_disp) > 30:
                            title_disp = title_disp[:30] + "..."
...
# 【シーン2】動画選択後（分析画面）
    ...
    if st.button("🚀 コメント分析を実行（上限100件）"):
        with st.spinner("コメントを取得してGPTで分析しています...（数十秒〜数分）"):
            ...
            if not comments:
                st.error("コメントを取得できませんでした（コメントが無効、またはAPI制限の可能性があります）")
            else:
                rows = []
                progress_bar = st.progress(0)
                
                # 並列処理（エラー回避のためWorker数を3に減らし安定化）
                with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
                    ...
                    for i, future in enumerate(concurrent.futures.as_completed(future_to_comment)):
                        ...
                        try:
                            ...
                            # エラーチェック (エラーでも行は追加し、エラー内容を表示する)
                            if isinstance(analysis, dict) and "error" in analysis:
                                row = {"コメント": c, "総合コメント": f"⚠️ 分析エラー: {analysis['error']}"}
                                # スコアはNoneのままにする
                                rows.append(row)
                            else:
                            ...
                            rows.append({"コメント": c, "総合コメント": f"⚠️ 処理例外: {str(e)}"})
...
if "analysis_df_raw" in st.session_state and st.session_state["analysis_df_raw"] is not None:
    ...
    for f in FEATURES:
        ...
        if score_col in df.columns:
            ...
            # 修正: スコアがNaN(分析失敗)の場合は、隠さずに表示する (条件を緩める)
            cond = s.isna() | ((s >= float(low)) & (s <= float(high)))
            mask &= cond
        else:
            mask &= True
    ...
    if len(df_filtered) > 0:
        # 1から始まるインデックス表示用の処理
        df_display = df_filtered[valid_cols].reset_index(drop=True)
        ...
        st.warning("表示できるコメントがありません。（フィルタ条件を緩めるか、再分析してください）")
