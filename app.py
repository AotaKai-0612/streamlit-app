...
def analyze_comment(comment_text):
    ...
    ## 2. 挑発性 (Provocation)
    皮肉、煽りなど、相手を感情的にさせる意図。
    - **0: なし**: 誠実でストレートな表現。
    - **1: 低**: 軽度の皮肉や嫌味。
    - **2: 中**: 上から目線、レッテル貼り。
    - **3: 高**: **相手を激昂させることを主目的とした強い煽り。**
    ...
    - **1: 低**: 根拠のない個人の感想。
    - **2: 中**: 具体的な指摘、改善提案。
    - **3: 高**: **非常に論理的で、独自の深い視点に基づき、議論に貢献するコメント。**
            ...
            for j in range(N_COLS):
                if i + j < len(videos):
                    ...
                    with cols[j]:
                        ...
                        # keyにunique_indexを追加
                        if st.button("選択", key=f"select_{v['video_id']}_{unique_index}"):
    ...
    if st.button("💬 コメント分析を実行（120件取得）"):
        with st.spinner("コメントを取得してGPTで分析しています...（数十秒〜数分）"):
            ...
            if not comments:
                ...
                st.success(f"✅ {len(df)} 件のコメントを分析しました。（画面表示は上位100件となります）")
