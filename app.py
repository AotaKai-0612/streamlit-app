...
# セッション状態の初期化（これが抜けていると検索結果が消えます）
...
if st.session_state["selected_video_id"] is None:
    ...
    if search_btn and query:
        st.session_state["search_results"] = []
        st.session_state["next_page_token"] = None
        results, token = search_videos(query, max_results=12) # 初回は12件取得
                    ...
                    for i, future in enumerate(concurrent.futures.as_completed(future_to_comment)):
                        ...
                        try:
                            ...
                            if isinstance(analysis, dict) and "error" in analysis:
                                # エラーは警告として出すが処理は止めない
                                st.warning(f"APIエラー: {analysis['error']}")
                            else:
