...
# 旧コードそのまま（ここを変えると動かなくなる可能性があるので維持）
    ...
    if st.button("💬 コメント分析を実行（上限100件）"):
        with st.spinner("コメントを取得してGPTで分析しています...（数十秒〜数分）"):
            ...
            if not comments:
                ...
                # 【旧コードと同じロジック】Worker数10、エラーはpass
                # これが一番動くとのことなので、下手にいじらずそのままにします。
...
if "analysis_df_raw" in st.session_state and st.session_state["analysis_df_raw"] is not None:
    ...
    for f in FEATURES:
        ...
        if score_col in df.columns:
            ...
            # 条件を少し緩める（NaNも一応表示候補に残すならここを調整、今回は旧コード通り）
